...
Monitors Front conversations tagged **Linked**, checks each referenced (linked) conversation for new activity, asks OpenAI whether the updates are significant, and posts a concise summary back to the parent conversation as an internal comment.
...
```bash
cp .env.example .env
# Fill in FRONT_API_KEY, LINKED_TAG_ID, OPENAI_API_KEY
...
| `OPENAI_MODEL` | | `gpt-4o-mini` | Model used for summarization |
...
4. Send the activity to OpenAI. It returns `{shouldPost, reasoning, message}`.
